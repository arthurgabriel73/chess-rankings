...
class TestListTopPlayersHistoriesUseCase:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (None, 5, 30, f'Invalid category: must be one of: {", ".join(CategoryValidator.valid_categories)}'),
            (7, 5, 30, f'Invalid category: must be one of: {", ".join(CategoryValidator.valid_categories)}'),
            ('', 5, 30, f'Invalid category: must be one of: {", ".join(CategoryValidator.valid_categories)}'),
            ('ab', 3, 30, f'Invalid category: must be one of: {", ".join(CategoryValidator.valid_categories)}'),
            ('a' * 101, 3, 30, f'Invalid category: must be one of: {", ".join(CategoryValidator.valid_categories)}'),
